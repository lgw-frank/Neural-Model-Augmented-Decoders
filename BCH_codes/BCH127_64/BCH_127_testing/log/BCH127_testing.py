...
sys.argv = "python 1.0 4.0 7 100 2000 8 BCH_127_78_7_strip.alist NMS-1".split()
...
GL.set_map('decoding_threshold',2000)

GL.set_map('regular_matrix',False)
GL.set_map('generate_extended_parity_check_matrix',False)
GL.set_map('reduction_iteration',4)
GL.set_map('redundancy_factor',2)
...
if decoder_type != 'SPA':
    ...
    if ckpt and ckpt.model_checkpoint_path: # ckpt.model_checkpoint_path means the latest ckpt
        ...
        basic_dir = '../BCH_'+str(n_dims)+'_training/ckpts/'+decoder_type+'/'+str(n_iteration)+'th'+'/'
        restore_ckpts_dir = basic_dir+'IF'+str(reduction_iteration)+'-'+str(redundancy_factor)+'/'
        #instance of Model creation
        test_Model = Decoder_module.Decoding_model()
        # saved restoring info
        checkpoint = tf.train.Checkpoint(myAwesomeModel=test_Model)
        ...
        if ckpt and ckpt.model_checkpoint_path: # ckpt.model_checkpoint_path means the latest ckpt
            ...
            print('Loading saved model from!',restore_ckpts_dir)
...
for SNR in SNRs[-1:]:
    ...
    Test_total_fer = 0.
    Test_total_ber = 0.
    Test_total_undected_ber = 0.
    #iput_file =  data_dir +'test-'+ending+str(snr)+'dB-'+suffix+'.tfrecord'
    iput_file =  '../Training_data_gen_'+str(n_dims)+'/data/snr4.0-4.0dB/'+'bch-train-nonzero.tfrecord'
    ...
    #     status.expect_partial()
    #     inputs = input_list[0]
    #     fer,undected_ber,ber,undetected_count,buffer = test_Model(inputs[0],inputs[1])
    # else:
    #     print('Expected model not found!')
    ...
    for i in range(num_counter):
        ...
        Test_total_undected_ber = Test_total_undected_ber+undected_ber
        ...
        if Test_total_fer > GL.get_map('decoding_threshold')/unit_batch_size:
          break
    average_test_fer = Test_total_fer/counter
    average_test_ber = Test_total_ber/counter
    average_undetected_fer = undetected_sum/(counter*unit_batch_size)
    average_undetected_ber = Test_total_undected_ber/counter
    tf.print(f'{counter}batches tested!')
    tf.print(f'SNR:{round(SNR,2)} FER:{average_test_fer:.4f} BER:{average_test_ber:.6f} \
    UFER:{average_undetected_fer:.6f} UBER:{average_undetected_ber:.6f}')

    FER_list.append((snr,round(float(average_test_fer),5)))
    BER_list.append((snr,round(float(average_test_ber),5)))
    UFER_list.append((snr,round(float(average_undetected_fer),5)))
    UBER_list.append((snr,round(float(average_undetected_ber),5)))
    with open(log_filename,'a+') as f:
      f.write('\nFor %.1fdB summary:\n'%round(SNR,2))
      f.write("FER %.4f, BER %.6f,UFER %.6f,UBER %.6f"%(average_test_fer,average_test_ber,average_undetected_fer,average_undetected_ber)+'\n')
...
with open(log_filename,'a+') as f:
  ...
  f.write(f"BER_list:{BER_list}\n")
  f.write(f"UFER_list:{UFER_list}\n")
  f.write(f"UBER_list:{UBER_list}\n")