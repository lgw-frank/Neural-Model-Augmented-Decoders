...
sys.argv = "python 3.0 7.0 9 100 2000 8 BCH_127_99_4_strip.alist NMS-1".split()
...
GL.set_map('record_interval',20)
GL.set_map('penalty_weight',2)
GL.set_map('num_shifts',3)
#GL.set_map('soft_margin',0.5)
...
if decoder_type != 'SPA':
    test_Model = Decoder_module.retore_saved_models()
else:
...
for SNR in SNRs:
    ...
    for i in range(num_counter):
        inputs = input_list[i]
        fer,ber,len_undetected,detected_indices,buffer = test_Model(inputs[0],inputs[1])
        print(test_Model.trainable_variables)
        Test_total_fer += fer
        Test_total_ber += ber
        buffer_inputs.append(buffer[0])
        buffer_labels.append(buffer[1])
        undetected_sum += len_undetected    
        counter += 1
        #print(test_Model.trainable_variables)
        if counter % 10 == 0:
          tf.print(f"{counter*unit_batch_size} codewords tested, FER:{Test_total_fer/counter:.8f}")
    ...
    average_undetected_ber = Test_total_uber/(counter*unit_batch_size*n_dims)
    average_undetected_rate = undetected_rate/counter
    tf.print(f'{counter}batches tested!')
    tf.print(f'SNR:{round(SNR,2)} FER:{average_test_fer:.8f} BER:{average_test_ber:.8f} \
             UFER:{average_undetected_fer:.8f} ')    
    FER_list.append((snr,round(float(average_test_fer),6)))
    BER_list.append((snr,round(float(average_test_ber),6)))
    UFER_list.append((snr,round(float(average_undetected_fer),6)))
    with open(log_filename,'a+') as f:
      f.write('\nFor %.1fdB summary:\n'%round(SNR,2))
      f.write("FER %.8f, BER %.8f,UFER %.8f"%(average_test_fer,average_test_ber,average_undetected_fer)+'\n')
    ...
    Decoder_module.save_decoded_data(updated_buffer,output_file,SNR,log_filename,list_length)
