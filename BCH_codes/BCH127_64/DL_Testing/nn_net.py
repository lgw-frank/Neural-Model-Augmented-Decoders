...
class conv_bitwise(keras.Model):
    def __init__(self):
        ...
        self.actual_length = list_length//2+1
        #self.actual_length = list_length
        with tf.init_scope():  # <-- CRITICAL FIX
            ...
            #self.cnv_three = layers.Conv1D(2,3,1,padding="valid",activation='linear',use_bias=False,name='2nd_layer')
    ...
    def preprocessing_inputs(self,input_slice):
        ...
        #file_input_data = self.substitute_inputs(file_input_data)
        #file_input_data = self.normalize_l2_preserve_sign(file_input_data)
        #file_input_data = self.signed_log_scaling(file_input_data)
        super_input_list = tf.transpose(file_input_data,perm=[1,0,2])
        #super_input_list = self.corrective_iteration(super_input_list)
        #file_input_data = tf.transpose(super_input_list,perm=[1,0,2])
        ...
        squashed_inputs = squashed_inputs[:,: self.actual_length,:]
        #reversed_tensor = tf.reverse(squashed_inputs, axis=[1])
        return squashed_inputs,labels,super_input_list 
    def corrective_iteration(self,soft_output_list):
        code = GL.get_map('code_parameters')
        new_soft_list = []
        for soft_output in soft_output_list:
            tmp = tf.cast(tf.where(soft_output>0,0,1),tf.int64)
            syndrome = tf.matmul(tmp,code.H,transpose_b=True)%2
            expanded_syndrome = tf.cast(tf.reduce_sum(tf.expand_dims(syndrome,-1)*code.H,1),tf.float32)
            scalor = tf.nn.softplus(self.scaling_factor)
            new_soft_output = soft_output * (1.0 - (expanded_syndrome* scalor)) 
            new_soft_list.append(new_soft_output)
        return tf.stack(new_soft_list,axis=0)
    def substitute_inputs(self,chunks):
    # chunks of `r` rows: [num_chunks, r, feature_dim]    
    # Get first row of each chunk and repeat it for broadcasting
        first_rows = chunks[:, :1, :]  # Shape: [num_chunks, 1, feature_dim]      
        # Compute row[i]/row[i-1] for all rows (skip first row)
        eps = 1e-8
        quotients = chunks[:, 1:, :] / (chunks[:, :-1, :] + eps)
        # Replace rows 1 to r-1 with: first_row * (row[i]/row[i-1])
        updated_chunks = tf.concat([
            first_rows,  # Keep first row unchanged
            first_rows * quotients  # Replace other rows
        ], axis=1)        
        # Reshape back to original structure
        return updated_chunks
    
    def normalize_l2_preserve_sign(self,tensor, axis=-1, epsilon=1e-12):
        """Normalizes tensor along `axis` by L2 norm without changing signs."""
        norms = tf.norm(tensor, ord=2, axis=axis, keepdims=True)
        # Avoid division by zero (optional, adjust epsilon as needed)
        safe_norms = tf.maximum(norms, epsilon)
        return tensor / safe_norms

    def signed_log_scaling(self,x):
        sign = tf.sign(x)
        log_values = tf.math.log1p(tf.abs(x))
        return sign * log_values
...
class osd_arbitrator(tf.keras.Model):
    def __init__(self, win_width):
        super(osd_arbitrator, self).__init__()
        self.input_width = win_width + 1
        
    def build(self, input_shape):
        # Remove input_shape from Dense layer
    ...
    def print_model(self):
        ...
        outputs = self.call(inputs)  # Use the call method to maintain consistency
        model = tf.keras.Model(inputs=inputs, outputs=outputs)
        model.summary()
        ...
        if not os.path.exists(fig_dir):
            os.makedirs(fig_dir)
        dot_img_file = fig_dir + 'osd_arbitrator_' + str(self.input_width) + '.h5'
        model.save(dot_img_file)
        
    def obtain_paras(self):
        total_params = tf.reduce_sum([tf.reduce_prod(var.shape) for var in self.trainable_variables])
        tf.print('Total params:', total_params)   
