...
def Testing_OSD(OSD_instance, snr, selected_ds, model_list, logistics):
    ...
    for i, input_data in enumerate(input_list):
        ...
        if config['DIA_indicator']:
            ...
            #new_inputs = forged_output + iterative_list[0]
        else:
        ...
        # OSD processing
        correct, fail, windows,swa_counts, complexity = OSD_instance.choose_decoding_strategy(
        ...
        counters.update({
            ...
            'swa_counts': counters['swa_counts'] + swa_counts,
...
def _report_progress(snr, counters, config, start_time):
    ...
    swa_counts_size = round(counters['swa_counts'] / counters['actual_size'], 4)
    ...
    print(f'--> S/F:{counters["correct"]} /{counters["fail"]} '
          f'Avr TEPs:{avg_size} Wins:{wins_size} SWA_counts:{swa_counts_size}')
