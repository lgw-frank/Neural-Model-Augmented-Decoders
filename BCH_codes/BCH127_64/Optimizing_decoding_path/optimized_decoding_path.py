...
def summary_ranking(ranked_list,error_pattern_batch_list):
    ...
    for i in range(len(batches)):
        ...
        if (i+1) % print_interval == 0:
            print(f'{i+1}th rank_sum:{rank_sum} avr_sum:{rank_sum/size_sum:.4f} obr:{out_bound_sum/size_sum:.4f}')
...
def drawing_plot(snr,data_counter_list,low_limit,save_path=None):
    ...
    for i, tuple_node in enumerate(data_counter_list):
        ...
        print(f'Counter{i+1}: rank_sum:{rank_sum} Size_sum:{size_sum} avr_sum:{rank_sum/size_sum:.4f} obs:{out_bound_sum} obr:{out_bound_sum/size_sum:.4f}')          
    ...
    plt.ylabel('Complementary  Cumulative Coverage Rate')
    plt.title(f'CCCR at SNR={snr}dB') #Complementary Cumulative Distribution Curves
