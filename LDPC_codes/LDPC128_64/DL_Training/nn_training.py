...
def Training_dia_model(DIA_model,base_ds,restore_info):
    list_length = GL.get_map('num_iterations')+1
    observor_length = list_length//2+1
    input_dims=3
    ...
    selected_ds = GL.build_training_dataset()
    iterator = iter(selected_ds) 
    #unpack related info for restoraging
    DIA_model,optimizer,logger_info,batch_index = Predict.NN_gen(DIA_model,restore_info,observor_length,input_dims)
...
def Testing_dia_model(DIA_model,base_ds): 
    ...
    if GL.get_map('DIA_deployment'):
        ...
        ending = ''
    print(f'\nSummary of ALMLT(option:{option_without_dia} no DIA) decoding_pth:')
    for key, value in sorted_items_initial:
         print(f'{key}: {value}',end=' ')
    print(f'\nSummary of ALMLT(option:{option_with_dia},DIA) decoding_pth:')
