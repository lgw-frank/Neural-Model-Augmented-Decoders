...
class Decoding_model(tf.keras.Model):
    ...
    def call(self,inputs): 
        soft_output_list,label = self.decoder_layer(inputs)
        return soft_output_list,label
...
class Decoder_Layer(tf.keras.layers.Layer):
    ...
    def belief_propagation_op(self, soft_input, labels):
        ...
        def condition(soft_output_array, cv_matrix,labels, iteration):
            return iteration < self.num_iterations
        def body(soft_output_array, cv_matrix,labels, iteration):
            ...
            iteration += 1   
            return soft_output_array,cv_matrix,labels,iteration
    
        soft_output_array, cv_matrix,labels,iteration = tf.while_loop(
            condition,
            body,
            loop_vars=[soft_output_array, cv_matrix,labels, 0]
        )   
