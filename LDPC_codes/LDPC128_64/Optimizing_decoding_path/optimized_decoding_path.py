...
def ALMLT_ranking(snr,macro_length):
    code = GL.get_map('code_parameters')
    theory_ordered_mean = theoretical_ordered_statistics_mean(snr)
    top_sequences = get_top_sequences_dp(theory_ordered_mean[-code.k:],macro_length)
    return top_sequences
    
def get_top_sequences_dp(sorted_reliability,macro_length):
    code = GL.get_map('code_parameters')
    ...
    for index_k in range(1, code.k + 1):
        ...
        dp[index_k] = new_sequences[:macro_length]
...
def retrieve_model_file(restore_model_info):
...
def reload_saved_model(restore_info):
    DIA_model = NN_struct.conv_bitwise()
    # Explicitly build the model with dummy input
    dummy_input = tf.zeros([1, DIA_model.actual_length,1])
    _ = DIA_model(dummy_input)  # Forces weight creation
    ...
    checkpoint = tf.train.Checkpoint(myAwesomeModel=DIA_model)
    #unpack related info for restoraging
    [ckpts_dir,ckpt_nm,restore_step] = restore_info  
    if restore_step:
        ckpt_f = retrieve_model_file(restore_info)
...
def find_error_pattern(input_iterator):
    ...
    if GL.get_map('DIA_deployment'):
        ...
        DIA_model = reload_saved_model(restore_info)
...
def query_error_patterns_dia(osd_model,dia_model,data_list):
    ...
    for batch in data_list:
        ...
        updated_inputs = dia_outputs+super_input_matrix[0]    
        element = osd_model.convention_osd_preprocess(updated_inputs,labels)
...
def generate_error_patterns(code_dims):
    """
    ...
    error_patterns = []
    counter = 0
    k = 0       # k is the number of 1s (Hamming weight)
    # Generate all combinations of positions for 1s
    while True:
        for positions in itertools.combinations(range(code_dims), k):
            # Create a list of integers representing the binary sequence
            sequence = [0] * code_dims
            for pos in positions:
                sequence[pos] = 1
            error_patterns.append(sequence)
            counter += 1
            if counter >= macro_length:
                jump_indicator = True
                break
        if jump_indicator:
            break
        ...
        for error_pattern_batch in batches:
            for key in error_pattern_batch:
                if tuple(key) in self.pattern_to_index:
                    ...
                    outlier_count += 1
        if outlier_count >= min(len(self.sorted_patterns),(relax_factor-1)*intercept_length):
            print('Too many outliers exist, enlarge relax factor or max oder-p of OSD!')
            sys.exit(-1)
    def update_teps_ordering(self):
        tuple_list = []
        for key in self.pattern_to_index.keys():
            tuple_element = (key, self.pattern_to_index[key][0]-self.pattern_to_index[key][1])
            tuple_list.append(tuple_element)
        self.sorted_patterns = [key for key,_ in sorted(tuple_list, key=lambda x: x[1])]
...
def summary_ranking(ranked_list,error_pattern_batch_list):
    ...
    for i in range(len(batches)):
        ...
        rank = manager.calculate_rank_sum(error_pattern_batch)
        rank_sum += rank
        if (i+1) % print_interval == 0:
            print(f'{i+1}th rank_sum:{rank_sum} avr_sum:{rank_sum/size_sum:.4f}')
    return data_counter,rank_sum,size_sum
...
def drawing_plot(snr,data_counter_list,low_limit,save_path=None):
    ...
    for i, tuple_node in enumerate(data_counter_list):
        node_counter,rank_sum,size_sum = tuple_node
        print(f'Counter{i+1}: rank_sum:{rank_sum} Size_sum:{size_sum} avr_sum:{rank_sum/size_sum:.4f}')          
    ...
    plt.title(f'ROR at SNR={snr}dB') #Complementary  Cumulative Coverage Rate
