...
def plot_curve(data_lists, label_list, snr, x_vertical_line=None):
    ...
    if x_vertical_line is not None:
        plt.axvline(
            x=x_vertical_line, 
            color='magenta', 
    ...
    # Set axis labels
    plt.xlabel('Ordered index', fontsize=16)
    plt.ylabel('Drifting delta', fontsize=16)
    ...
    plt.savefig(
        f'{gen_data_dir}mean_reliability_{snr}dB_ldpc_{code.check_matrix_column}_{code.k}_code.jpg',
...
def Testing_pro_bits(snr,selected_ds):
    ...
    actual_size = 0 
    sorted_sum = np.zeros([1,code.check_matrix_column],dtype=float)
    swapped_sum = np.zeros([1,code.check_matrix_column],dtype=float)
    sorted_sum = np.zeros([1,code.check_matrix_column],dtype=float)
    swapped_sum = np.zeros([1,code.check_matrix_column],dtype=float)
