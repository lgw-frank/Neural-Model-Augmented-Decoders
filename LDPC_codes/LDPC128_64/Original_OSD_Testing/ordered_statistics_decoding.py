...
class osd:
    ...
    def check_matrix_reorder(self,inputs,labels,sorted_sum):
        expanded_H = tf.expand_dims(self.H,axis=0)
        ...
        sorted_sum = self.ordered_statistics_sum(order_inputs, sorted_sum)
        ...
        return order_H_list,order_inputs,order_labels,sorted_sum 
    
    def ordered_statistics_sum(self,sorted_inputs,sorted_sum):
        sorted_sum += tf.reduce_sum(abs(sorted_inputs),axis=0)
        return sorted_sum
    ...
    def identify_mrb(self,order_H_list):
        ...
        for i in range(order_H_list.shape[0]):
            ...
            updated_LRB = index_order[:code.check_matrix_column-code.k]
            ...
            jump_point = (code.check_matrix_column-code.k)-4*maximum_order
    ...
    def acquire_min(self,error_pattern_matrix,initial_mrb,M_matrix,order_hard,mag_metric): #minimum of teps belonging to some order pattern
    ...
    def convention_osd_preprocess(self,inputs,labels,sum_list):
        code = GL.get_map('code_parameters')   
        order_list= self.check_matrix_reorder(inputs,labels,sum_list[0])
        order_H_list,order_inputs,order_labels,sum_list[0] = order_list
        ...
        swapped_list = []
        for i in range(input_size):
            swapped_part_one = tf.sort(abs(order_input_list[i][:code.check_matrix_column-code.k]))
            swapped_element = tf.concat([swapped_part_one,abs(order_input_list[i][-code.k:])],axis=0)
            swapped_list.append(swapped_element)
        sum_list[1] += tf.reduce_sum(tf.reshape(swapped_list,[-1,code.check_matrix_column]),axis=0)        
        return sum_list
    
    def convention_osd(self,inputs,labels,teps_matrix,sum_list):
        code = GL.get_map('code_parameters')   
        success_dec = 0
        failure_dec = 0
        order_list= self.check_matrix_reorder(inputs,labels,sum_list[0])
        ...
        for i in range(input_size):
            ...
            discrepancy_truth = tf.cast((order_label_list[i] + order_hard)%2,dtype=tf.float32)
            mag_metric = abs(order_input_list[i])
            ...
            min_sum = self.acquire_min(teps_matrix,initial_mrb,M_matrix,order_hard,mag_metric)
            if min_sum == discrepancy_sum_truth:
                success_dec += 1
            else:
                failure_dec += 1
            complexity_sum += teps_matrix.shape[0]
        return success_dec,failure_dec,complexity_sum,sum_list
