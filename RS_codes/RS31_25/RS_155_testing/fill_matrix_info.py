...
class Code:
    ...
    def load_code(self,file_path):
        ...
        if GL.get_map('regular_matrix'):
           ...
           absolute_head = '../RS_'+str(self.n)+'_training/ckpts/'
           ...
           if GL.get_map('generate_extended_parity_check_matrix') and (not os.path.exists(saved_file)):
               ...
               full_rank_matrix = self.add_cyclic_shifts_to_full_rank(unique_H_list,self.n-self.k)
               redundancy_factor = GL.get_map('redundancy_factor') #redundancy multiplier              
               gap_rows = int(redundancy_factor*(self.n-self.k))-full_rank_matrix.shape[0]
    ...
    def simulated_annealing_row_operations(self,H, initial_temp, cooling_rate, num_iterations, beta):
       ...
       for i in range(num_iterations):
           ...
           shift_amount = np.random.randint(1, new_H.shape[1]//step_size)
           new_H[row] = np.roll(new_H[row], shift_amount*step_size)
    ...
    def reduce_matrix_rank_preserving(self,matrix):
        """
        ...
        for i in range(len(reduced_matrix)-1, -1, -1):
            ...
            if len(reduced_matrix) == original_rank:
                break
    ...
    def lexicographically_smallest_rotation(self,row):
        ...
        step_size = GL.get_map('extend_F')
        rotations = [np.roll(row, i*step_size) for i in range(len(row)//step_size)]
    ...
    def iterate_for_min_weight_rows(self,matrix_cmp,range_start):
        ...
        for shift in range(matrix_cmp.shape[1]//step_size):          
            matrix_sum = (base_row + np.roll(matrix_cmp,shift*step_size,axis=1))%2
    ...
    def add_cyclic_shifts_to_full_rank(self,matrix_list,target_rank):
        shrinked_matrix = matrix_list[-1]
        current_rank = self.detect_rank(shrinked_matrix)
        shrinked_matrix = self.sort_rows_by_weight(shrinked_matrix)
        base_matrix = self.reduce_matrix_rank_preserving(shrinked_matrix)
        if current_rank < target_rank:
            early_terminator = False
            for matrix in reversed(matrix_list):  
                dilated_matrix = self.generate_all_cyclic_shifts(matrix)
    ...
    def group_rows_by_weight_acquire_rank(self,matrix):
        ...
        for weight in np.sort(np.unique(row_weights)):
            ...
            weight_groups[weight] = matrix[indices]    
        # Print results
