...
def global_setting(argv):
    ...
    set_map('nms_termination_step',300)
    ...
    set_map('extend_F',5)  #depending on chosen code
...
def logistic_setting(original_NMS_indicator=False):
    ...
    if original_NMS_indicator:
        basic_dir = './convention_ckpts/'+decoder_type+'/'+str(n_iteration)+'th'+'/'
    else:       
        basic_dir = './ckpts/'+decoder_type+'/'+str(n_iteration)+'th'+'/'
    reduction_iteration = get_map('reduction_iteration')
    redundancy_factor = get_map('redundancy_factor')   
    ckpt_nm = f'{prefix_str}-ckpt'
    intermediate_dir = basic_dir+'IF'+str(reduction_iteration)+'-'+str(redundancy_factor)+'/'
...
def data_setting(code,unit_batch_size):
    ...
    #dataset_train = dataset_train.take(200)
