...
class Decoder_Layer(tf.keras.layers.Layer):
    ...
    def aggregate_cyclic_words(self,soft_input):
        ...
        if num_shifts > 0:
            shift_limit = soft_input.shape[1]//step_size
            shifted_integers = self.get_evenly_shifted_integers(shift_limit,num_shifts)
            shifted_input_list = [tf.roll(soft_input,shifted_integers[i]*step_size,axis=1) for i in range(num_shifts)]
    ...
    def belief_propagation_op(self, soft_input, labels):
        ...
        # sigma = np.sqrt(1. / (2 * (float(self.code.k)/float(self.code.n)) * 10**(snr_lo/10)))
    ...
    def compute_cv1(self,vc_matrix):
        vc_matrix = tf.clip_by_value(vc_matrix, -20, 20)
    ...
    def marginalize(self,cv_matrix, soft_input,shifted_integers,iteration,soft_output_array):
        ...
        if num_shifts > 0:
            ...
            restored_list = [tf.roll(temp[i*batch_size:(i+1)*batch_size], shift=-shifted_integers[i]*step_size, axis=-1) for i in range(num_shifts)]
            delta_result = tf.add_n(restored_list)
            soft_output = soft_output_array.read(iteration)+delta_result
        else:
...
#postprocessing after first stage training
def postprocess_training(NMS_model, iterator):
    code = GL.get_map('code_parameters')
    #collecting erroneous decoding info
    buffer_inputs = []
    buffer_labels = []
    ...
    for i in range(num_counter):
        ...
        inputs = input_list[i]
        soft_output_list,label,_ = NMS_model(inputs)
        FER_count,BER_count,delare_n_index = NMS_model.get_eval(soft_output_list,label)     
        buffer_inputs_tmp,buffer_labels_tmp = NMS_model.collect_failed_input_output(soft_output_list,label,delare_n_index)   
...
def training_block(batch_index, Model, optimizer, exponential_decay, selected_ds, log_info, restore_info):
    ...
    while True:
        ...
        if batch_index % print_interval == 0:
            ...
            print("Current parameters' evaluation:")
