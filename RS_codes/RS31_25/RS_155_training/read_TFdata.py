...
def feature_selection(feature_indicator):
    ...
    if feature_indicator == 'one':  
        ...
        code_length = code.n
        num_iterations = GL.get_map('num_iterations')+1 
        feature_description = {
            'feature': tf.io.FixedLenFeature([num_iterations], tf.float32),
            'label': tf.io.FixedLenFeature([code_length], tf.int64),
            'shape': tf.io.FixedLenFeature([], tf.int64)
        }
...
def data_handler(code_length, file_name,  batch_size=1,feature_indicator='zero'): 
